...
PL_read_data <-
  ...
  if (length(which(footballR::year_dates$start_date >= from)) > 0) {
    first_season <- min(which(footballR::year_dates$start_date >= from))
  }else{
    first_season <- 1
  }

  # Get the end season
  if (length(which(footballR::year_dates$end_date >= to)) > 0) {
    last_season <- min(which(footballR::year_dates$end_date >= to))
  }else{
    last_season <- nrow(footballR::year_dates)
  }

  years_to_get <- footballR::year_dates$years[first_season:last_season]  # Years to read in
